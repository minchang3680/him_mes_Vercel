...
export default function MachineFaultTimelineClient() {
  ...
  const machineGrouped = selectedMachines.map((machine, index) => {
    ...
    const sortedItems = filtered
      .filter((d) => d.machine_name.trim().toLowerCase() === machine.trim().toLowerCase())
      .sort((a, b) => new Date(a.detected_at).getTime() - new Date(b.detected_at).getTime());

    let prevType: number | null = null;

    const dataPoints = sortedItems.map((d) => {
  ...
  const chartOptions: ChartOptions<'line'> = {
    ...
    scales: {
      y: {
        ...
        max: 3.5,
        ticks: {
          callback: (val) => faultTypeMap[val as number],
          stepSize: 1,
          font: { size: 14 },
        },
        title: {
      ...
      <div className="flex flex-col gap-6">
        {machineGrouped.map((machineChart, idx) => (
          <div key={idx} className="h-[400px] bg-white border rounded shadow p-4">
            ...
            <Line data={{ datasets: [machineChart] }} options={{ ...chartOptions, plugins: { ...chartOptions.plugins, legend: { display: false } } }} />
          </div>
        ))}
      </div>
    </main>
